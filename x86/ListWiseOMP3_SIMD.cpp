...
bool ReadIndex()
{
    ...
    int bstNum;
    #pragma omp parallel for schedule(dynamic), if(InParallel), num_threads(NUM_THREADS), private(bstNum)
...
void GetIntersectionAndAnswer() {
    ...
    __m128 cmp;
    char r;
    #pragma omp for schedule(guided), private(sect, x1, x2, cmp, r)
    for(sect = 0; sect <= rstLen - 4; sect += 4) {
        // Get Intersection
            ...
            if( !(r & andOp[k]) ) { // [sect + k]
                for(int i = 0; i < 32; i++) {
                    if(rst[sect + k].test(i)) {
                        ...
                        #pragma omp critical(resultCountA)
    ...
    // Remaining section
    #pragma omp for
    for(int rsect = rstLen - rstLen % 4; rsect < rstLen; rsect++) {
        // Get Intersection
        rst[rsect] = bindex[query[0]][rsect];
        for(int i = 1; i < numQuery; i++) {
            rst[rsect] &= bindex[query[i]][rsect];
        }
